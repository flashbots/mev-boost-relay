...
CREATE TABLE IF NOT EXISTS ` + TableBid + ` (
	...
	builder_block_submission_id bigint references ` + TableBuilderBlockSubmission + `(id)
);

CREATE UNIQUE INDEX IF NOT EXISTS ` + TableBid + `_slot_hash_pk_idx ON ` + TableBid + `(slot, parent_hash, proposer_pubkey);
...
CREATE TABLE IF NOT EXISTS ` + TableDeliveredPayload + ` (
	...
	builder_block_submission_id bigint references ` + TableBuilderBlockSubmission + `(id),
	builder_bid_id bigint references ` + TableBid + `(id),
